...
import com.baofoo.rsa.RsaReadUtil;
import com.baofoo.util.FormatUtil;
import com.zhph.channel.baofu.util.RsaConst;
...
public final class RsaCodingUtil {
	 ...
	 * @return
	 */
	public static String encryptByPriPfxFile(byte[] src, String pfxPath, String priKeyPass) {
	 ...
	 * @param privateKey
	 */
	public static String encryptByPrivateKey(byte[] src, PrivateKey privateKey) {
		byte[] destBytes = null;
		destBytes = rsaByPrivateKey(src, privateKey, Cipher.ENCRYPT_MODE);
		if (destBytes == null) {
			return null;
		}
	 ...
	 * @return
	 */
	public static byte[] decryptByPubCerFileByte(String src, String pubCerPath) {
		PublicKey publicKey = RsaReadUtil.getPublicKeyFromFile(pubCerPath);
		if (publicKey == null) {
			return null;
		}
		return decryptByPublicKeyByte(src, publicKey);
	}
	 ...
	 * @return
	 */
	public static byte[] decryptByPublicKeyByte(String src, PublicKey publicKey) {
		byte[] destBytes = rsaByPublicKey(FormatUtil.hex2Bytes(src), publicKey, Cipher.DECRYPT_MODE);
		if (destBytes == null) {
			return null;
		}
